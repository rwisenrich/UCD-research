# 06 — Measurement, Time, Causality, and Continuum

This layer records what follows exactly from the finite mathematics and what still requires physical promotion.
...
## Complex structure from C3 plus reversal
...
## Ordinal succession and metric time
...
Finite-depth local updates generate finite causal cones. A physically significant causal order requires more than bookkeeping order; this is isolated as `CONT-ORDER`.

## Continuum reconstruction program

The current continuum gate is decomposed into:

- `CONT-ORDER` — physical causal precedence;
- `CONT-MAN` — manifoldlike continuum convergence/approximation;
- `CONT-VOL` — a local volume element or event-count-to-volume law;
- `CONT-DIM` — dimension selection/inference from order-volume growth;
- `CONT-SCALE` — one absolute density/duration/length calibration.

Under suitable manifold and causality assumptions, causal order fixes the Lorentzian metric up to conformal factor, while a volume element fixes the remaining factor. This reduces the problem; it does not by itself prove that the UCD finite registry realizes those continuum premises.